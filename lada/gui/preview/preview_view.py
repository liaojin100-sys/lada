...
class PreviewView(Gtk.Widget):
    ...
    button_mute_unmute = Gtk.Template.Child()
       # === [新增] 对应 XML 里添加的 ID ===
    button_toggle_mosaic: Gtk.ToggleButton = Gtk.Template.Child() 
    # ...
    ...
    def button_play_pause_callback(self, button_clicked):
        ...
        if self.pipeline_manager.state == PipelineState.PLAYING:
            ...
            # ================= 插入开始 =================
    @Gtk.Template.Callback()
    def on_button_toggle_mosaic_toggled(self, button):
        # 检查是否已加载文件
        ...
        # 决定模型名称：开启则读取配置，关闭则设为 None
        target_model = self.config.mosaic_restoration_model if is_enabled else None
        
        # 更新 options，这会触发 setter 自动重启 worker
    ...
    def setup_config_signal_handlers(self):
        ...
        def on_mosaic_restoration_model(object, spec):
            if self._frame_restorer_options:
                 # === [修改] ===
                # 只有当开关开启时，才实时更新模型；否则只更新 Config 对象，不应用到当前播放
                if self.button_toggle_mosaic.get_active():
                ...
                # 如果开关是关的，我们不需要做动作，因为下次打开开关时会读取 config 中的新值
    ...
    def _open_file(self, file: Gio.File):
        # === [修改开始] ===
        # 获取按钮当前的视觉状态（如果 UI 还没初始化完，默认为 False）
        is_mosaic_enabled = self.button_toggle_mosaic.get_active()
        
        # 只有当按钮处于激活状态时，才加载配置中的 AI 模型
        initial_model = self.config.mosaic_restoration_model if is_mosaic_enabled else None
        
        # 创建 Options 时使用 initial_model
        self.frame_restorer_options = FrameRestorerOptions(
            initial_model,  # <--- 这里改用我们计算出的 initial_model
